...
module spiral_compiler =
    ...
    module BlockParsing =
        ...
        let skip' (d : Env) i = d.i.Value <- d.i.contents+i
        let skip d = skip' d 1
    ...
    module CodegenUtils =
        ...
        exception CodegenError of PartEvalPrepass.Range option * string
    ...
    module CodegenCuda =
        ...
        let codegen (default_env : Startup.DefaultEnv) (globals : _ ResizeArray, fwd_dcls : _ ResizeArray, types : _ ResizeArray, functions : _ ResizeArray, main_defs : _ ResizeArray) (env : PartEvalResult) =
            ...
            fun vs (x : TypedBind []) ->
                ...
                global' (IO.File.ReadAllText(IO.Path.Join(
                    // AppDomain.CurrentDomain.BaseDirectory,
                    IO.Path.Combine (SpiralFileSystem.get_workspace_root (), "deps/The-Spiral-Language/The Spiral Language 2"),
    ...
    module CodegenPython =
        ...
        let codegen (default_env : Startup.DefaultEnv) env x = 
            ...
            let codegen = CodegenCuda.codegen default_env ars env
            let python_code =
            ...
            cuda_kernels
                .AppendLine("\"\"\"")
                .AppendLine(IO.File.ReadAllText(IO.Path.Join(
#if !INTERACTIVE
                    // AppDomain.CurrentDomain.BaseDirectory,
                    IO.Path.Combine (SpiralFileSystem.get_workspace_root (), "deps/The-Spiral-Language/The Spiral Language 2"),
#else
    ...
    module WDiffPrepass =
        ...
        let rec prepass (package_id,module_id,path,env) = function
            | Cons((_,r,_) : TypecheckerStateValue, ls) ->
                ...
                let x = (PartEvalPrepass.prepass package_id module_id path env).filled_top filled_top
    ...
    module SignalRSupervisor =
        ...
        let show_position (s : SupervisorState) (x : PartEvalPrepass.Range) =
        ...
        let show_trace s (x : PartEval.Trace) (msg : string) =
            let rec loop (l : PartEval.Trace) = function
                | (x : PartEvalPrepass.Range) :: xs ->
                    match l with
                ...
                | BuildFile (x, res) ->
                    ...
                    let handle_build_result = function
                        | BuildOk l ->
                            Job.fromAsync (async {
                                for x in l do
                                    do! IO.File.WriteAllTextAsync(IO.Path.ChangeExtension(file,x.file_extension), x.code) |> Async.AwaitTask
                    ...
                    let file_build (s : SupervisorState) mid (tc : WDiff.ProjStateTC, prepass : WDiffPrepass.ProjStatePrepass) =
                        ...
                        Hopac.start (a.state >>= fun (has_error',_) ->
                            ...
                            Stream.foldFun (fun _ (_,_,env) -> env) PartEvalPrepass.top_env_empty x.result >>= fun env ->
                            let body() =
                                ...
                                | Some main ->
                                    ...
                                    try 
                                        let build codegen backend file_extension =
                                            let (a,_),b = PartEval.peval {prototypes_instances=prototypes_instances; nominals=nominals; backend=backend} main
                                            BuildOk [{|code = codegen b a; file_extension = file_extension|}]
                                        match backend with
                                        | "Fsharp" -> build CodegenFsharp.codegen "Fsharp" "fsx"
                                        | "C" -> build CodegenC.codegen "C" "c"
                                        | "Python + Cuda" -> build (CodegenPython.codegen default_env) "Python" "py"
                                    ...
                                    with
                                        | :? PartEval.TypeError as e -> BuildErrorTrace(show_trace s e.Data0 e.Data1)
    ...
    /// ### new_server
    let new_server () =
        let event = Event<SignalRSupervisor.ClientErrorsRes> ()
        ...
        let error_ch_create msg =
            ...
            Hopac.server (Job.forever (Ch.take x >>= (
                msg >> fun (x : SignalRSupervisor.ClientErrorsRes) ->
                    Hopac.Job.awaitUnitTask (
                        task {
                            event.Trigger x
                            trace Verbose (fun () -> $"SignalRSupervisor.error_ch_create / x: %A{x}") (fun () -> "")
                            ()
                        }
        ...
        let errors : SignalRSupervisor.SupervisorErrorSources = {
            fatal = error_ch_create SignalRSupervisor.FatalError
            package = error_ch_create SignalRSupervisor.PackageErrors
            tokenizer = error_ch_create SignalRSupervisor.TokenizerErrors
            parser = error_ch_create SignalRSupervisor.ParserErrors
            typer = error_ch_create SignalRSupervisor.TypeErrors
            traced = error_ch_create SignalRSupervisor.TracedError
            }
        let supervisor = Ch()
        let atten = Ch()

        do Hopac.server (SignalRSupervisor.attention_server errors atten)

        let args = [| "--port"; "0" |]
        let env = Startup.parse args
        do Hopac.start (SignalRSupervisor.supervisor_server env atten errors supervisor)
