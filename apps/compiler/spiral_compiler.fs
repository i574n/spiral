    ...
    let codegenLua (env : PartEvalResult) (x : TypedBind []) =
        ...
        and op s d a =
            ...
            | TyUnionUnbox(is,x,on_succs,on_fail) ->
                complex <| fun s ->
                let case_tags = x.Item.tags
                let vs = is |> List.map (fun (L(i,_)) -> sprintf "v%i" i) |> String.concat ", "
                ...
                Map.iter (fun k (a,bnds) ->
                    ...
                    let guard =
                        a
                        |> List.mapi (fun idx _ -> sprintf "__v[%i].tag == \"%si%i\"" (idx+1) prefix i)
                        |> String.concat " and "
                    if first then
                ...
                match on_fail with
                | Some b -> line s "else"; binds (indent s) b; line s "end"
                | None -> line s "end"
            | TyUnionBox(a,b,c) ->
                let c = c.Item
                let i = c.tags.[a]
                let vars =
                    match data_term_vars b with
                    | [||] -> ""
                    | x -> Array.mapi (fun j v -> sprintf "_%i = %s" (j+1) (show_w v)) x |> String.concat ", " |> sprintf ", %s"
                match c.layout with
                | UHeap -> sprintf "{ tag = \"Uh%ii%i\"%s }" (uheap c.cases).tag i vars
                | UStack -> sprintf "{ tag = \"Us%ii%i\"%s }" (ustack c.cases).tag i vars
                |> simple
            | TyToLayout(a,b) ->
                let a = layout_vars a
                match b with
        ...
        and closure : _ -> ClosureRecLua =
            jp
                ...
                if x.free_vars.Length > 0 then line (indent s) (sprintf "local %s = table.unpack(capt)" (x.free_vars |> Array.map (fun (L(i,_)) -> sprintf "v%i" i) |> String.concat ", "))
                match x.domain_args with
                | [||] ->
                ...
                | many ->
                    line (indent s) "return function(dom)"
                    line (indent (indent s)) "local _ = table.unpack(dom)"
                binds (indent (indent s)) x.body
                line (indent s) "end"
                line s "end"
            )
